...
Item {
  ...
  readonly property var mediaActivePlayer: selectMediaPlayer()
  readonly property bool hasMedia: mediaActivePlayer !== null
  readonly property string mediaTitle: mediaActivePlayer ? String(mediaActivePlayer.trackTitle || "") : ""
  readonly property string mediaArtist: mediaActivePlayer ? String(mediaActivePlayer.trackArtist || "") : ""

  function hasTrackMetadata(player) {
    if (!player) return false
    return (player.trackTitle && String(player.trackTitle).length > 0) || (player.trackArtist && String(player.trackArtist).length > 0)
  }
  ...
  function selectMediaPlayer() {
    ...
    for (var i = 0; i < root.mediaPlayers.length; i++) {
      ...
      if (!p || !root.hasTrackMetadata(p)) continue
      if (!any) any = p
      if (p.isPlaying) { playing = p; break }
    }
    return playing || any || null
  }
  ...
  Process {
    id: faceCheckProc
    command: ["bash", "-c", "if [[ -f /etc/pam.d/omarchy-lock-face && -f \"$HOME/.config/omarchy-face-lock/enrolled\" ]]; then echo yes; else echo no; fi"]
